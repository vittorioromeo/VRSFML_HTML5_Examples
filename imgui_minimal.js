...
function findWasmBinary() {
  return locateFile("imgui_minimal.wasm");
}
...
function _eglDestroySurface(display, surface) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(14, 0, 1, display, surface);
  if (display != 62e3) {
    EGL.setErrorCode(12296);
    return 0;
  }
...
function _eglGetConfigAttrib(display, config, attribute, value) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(15, 0, 1, display, config, attribute, value);
  if (display != 62e3) {
    EGL.setErrorCode(12296);
    return 0;
  }
...
function _eglInitialize(display, majorVersion, minorVersion) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(18, 0, 1, display, majorVersion, minorVersion);
  if (display != 62e3) {
    EGL.setErrorCode(12296);
    return 0;
  }
...
function _eglMakeCurrent(display, draw, read, context) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(19, 0, 1, display, draw, read, context);
  if (display != 62e3) {
    EGL.setErrorCode(12296);
    return 0;
  }
...
function _eglSwapBuffers(dpy, surface) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(20, 0, 1, dpy, surface);
...
function _emscripten_get_fullscreen_status(fullscreenStatus) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(27, 0, 1, fullscreenStatus);
  if (!JSEvents.fullscreenEnabled()) return -1;
...
var ASM_CONSTS = {
  ...
  525819: () => Module["SDL3"]["mouse_x"],
  525857: () => Module["SDL3"]["mouse_y"],
  525895: $0 => Module["SDL3"]["mouse_buttons"][$0],
  525943: $0 => {
    var data = $0;
  ...
  528187: () => document.body.clientWidth,
  528225: () => document.body.clientHeight,
  528264: () => window.innerWidth,
  528294: () => window.innerHeight,
  528325: () => window.outerWidth,
  528355: () => window.outerHeight,
  528386: () => window.pageXOffset,
  528417: () => window.pageYOffset,
  528448: ($0, $1) => {
  ...
  534726: () => window.innerWidth,
  534756: () => window.innerHeight,
  534787: $0 => {
  ...
  537253: () => window.innerWidth,
  537283: () => window.innerHeight,
  537314: $0 => {
