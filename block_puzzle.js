...
function findWasmBinary() {
  return locateFile("block_puzzle.wasm");
}
...
function __mmap_js(len, prot, flags, fd, offset, allocated, addr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(11, 0, 1, len, prot, flags, fd, offset, allocated, addr);
...
function __munmap_js(addr, len, prot, flags, fd, offset) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(12, 0, 1, addr, len, prot, flags, fd, offset);
  offset = bigintToI53Checked(offset);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
    if (prot & 2) {
...
function __setitimer_js(which, timeout_ms) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(13, 0, 1, which, timeout_ms);
...
function _emscripten_get_fullscreen_status(fullscreenStatus) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(30, 0, 1, fullscreenStatus);
  if (!JSEvents.fullscreenEnabled()) return -1;
...
function _emscripten_get_screen_size(width, height) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(33, 0, 1, width, height);
...
function _emscripten_request_fullscreen_strategy(target, deferUntilInEventHandler, fullscreenStrategy) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(34, 0, 1, target, deferUntilInEventHandler, fullscreenStrategy);
  var strategy = {
...
function _emscripten_set_window_title(title) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(52, 0, 1, title);
...
var ASM_CONSTS = {
  ...
  2343502: () => Module["SDL3"]["mouse_x"],
  2343540: () => Module["SDL3"]["mouse_y"],
  2343578: $0 => Module["SDL3"]["mouse_buttons"][$0],
  2343626: $0 => {
    var data = $0;
  ...
  2345870: () => document.body.clientWidth,
  2345908: () => document.body.clientHeight,
  2345947: () => window.innerWidth,
  2345977: () => window.innerHeight,
  2346008: () => window.outerWidth,
  2346038: () => window.outerHeight,
  2346069: () => window.pageXOffset,
  2346100: () => window.pageYOffset,
  2346131: ($0, $1) => {
  ...
  2352409: () => window.innerWidth,
  2352439: () => window.innerHeight,
  2352470: $0 => {
  ...
  2354942: () => window.innerWidth,
  2354972: () => window.innerHeight,
  2355003: $0 => {
  ...
  2360991: () => (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined),
  2361095: () => {
    try {
  ...
  2361266: ($0, $1, $2, $3, $4, $5) => {
    ...
    device.scriptNode.onaudioprocess = function(e) {
      if (device.intermediaryBufferView == null || device.intermediaryBufferView.length == 0) {
        device.intermediaryBufferView = new Float32Array((growMemViews(), HEAPF32).buffer, pIntermediaryBuffer, bufferSize * channels);
      }
  ...
  2364143: $0 => window.miniaudio.get_device_by_index($0).webaudio.sampleRate,
  2364216: $0 => {
